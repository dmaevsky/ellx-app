...
module.exports = {
  ...
  variants: {
    ...
    borderColor: ['dark', 'focus', 'dark-focus', 'hover'],
  ...
  plugins: [
    function dark({ addVariant, e }) {
      const d = '.mode-dark';

      addVariant('dark', ({ modifySelectors, separator }) => {
        modifySelectors(({ className }) => `${d} .${e(`dark${separator}${className}`)}`);
      });

      addVariant('dark-hover', ({ modifySelectors, separator }) => {
        modifySelectors(({ className }) => `${d} .${e(`dark-hover${separator}${className}`)}:hover`);
      });

      addVariant('dark-focus', ({ modifySelectors, separator }) => {
        modifySelectors(({ className }) => `${d} .${e(`dark-focus${separator}${className}`)}:focus`);
      });
    }
  ],
};
