...
<div id="ellx-node-navigator"
  ...
  on:mouseleave={() => { deps = [] }}
  on:mousedown={(e) => {if (e.target.id !== "ellx-node-search") e.preventDefault()}}
>
  <div class="z-50 sticky top-0 pl-2 pr-4 pt-4 flex flex-row w-full gap-4 items-center bg-gray-100 dark:bg-gray-900">
    <input id="ellx-node-search" class="w-full h-6 px-2 text-opacity-100 bg-white border border-gray-500 border-opacity-40 rounded-sm dark:border dark:border-white dark:border-opacity-40 dark:bg-gray-900"
    ...
    <div id="ellx-node-nav-toggle"
         ...
         on:click={() => nodeNavigatorOpen.set(false)}>
